function varargout = dve_rectangleFTEsearch_V9_35(varargin)
% DVE_RECTANGLEFTESEARCH_V9_35 MATLAB code for dve_rectangleFTEsearch_V9_35.fig
%      DVE_RECTANGLEFTESEARCH_V9_35, by itself, creates a new DVE_RECTANGLEFTESEARCH_V9_35 or raises the existing
%      singleton*.
%
%      H = DVE_RECTANGLEFTESEARCH_V9_35 returns the handle to a new DVE_RECTANGLEFTESEARCH_V9_35 or the handle to
%      the existing singleton*.
%
%      DVE_RECTANGLEFTESEARCH_V9_35('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in DVE_RECTANGLEFTESEARCH_V9_35.M with the given input arguments.
%
%      DVE_RECTANGLEFTESEARCH_V9_35('Property','Value',...) creates a new DVE_RECTANGLEFTESEARCH_V9_35 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before dve_rectangleFTEsearch_V9_35_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to dve_rectangleFTEsearch_V9_35_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help dve_rectangleFTEsearch_V9_35

% Last Modified by GUIDE v2.5 25-May-2012 12:37:33

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @dve_rectangleFTEsearch_V9_35_OpeningFcn, ...
                   'gui_OutputFcn',  @dve_rectangleFTEsearch_V9_35_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before dve_rectangleFTEsearch_V9_35 is made visible.
function dve_rectangleFTEsearch_V9_35_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to dve_rectangleFTEsearch_V9_35 (see VARARGIN)

% Choose default command line output for dve_rectangleFTEsearch_V9_35
...
% UIWAIT makes dve_rectangleFTEsearch_V9_35 wait for user response (see UIRESUME)
% uiwait(handles.fig1);


% --- Outputs from this function are returned to the command line.
function varargout = dve_rectangleFTEsearch_V9_35_OutputFcn(hObject, eventdata, handles) 
...
%================%
    ...
    handles.headera = strcat('FTE start: ',datestr(handles.fdna), ' NEV: '...
        , num2str(handles.nev),', w:', num2str(handles.w), ', ny:', num2str(handles.ny));
    handles.headerb = strcat('FTE stop: ',datestr(handles.fdnb), ', sc:'...
        , handles.sc, ', src:', handles.src);
...
function handles = plotstuff(hObject, eventdata,handles)
    ...
    % polyplot
    handles = plot_data_calc(hObject,eventdata,handles);
    set(gcf,'CurrentAxes',handles.axes1)
    pcolor(handles.x*handles.L0,handles.y(handles.gg)*handles.L0,handles.B3);            % displays 3rd axis (bz) in faceted shading
    shading interp                       % changes shading to interpolated
    minb3=min(min(handles.B3));
...
anr = round(handles.An/0.01);       % rounded handles.An1 (2 dec)
axr = round(handles.ax1/0.01);      % rounded ax1 (2 decimals)
...
function polynums(hObject,eventdata, handles)
    ...
    % Stefan and Dave's Code (obsolete)
    % --------
    %     iidx = handles.ndata-1;
    %     if handles.polyamin1 == 0
    %         midx = round(handles.An(2:iidx)/0.01);
    ...
    % Stefan and Dave's Code (production)
    % ---------
    %     ydiff = abs(handles.polyyys1(idx)' - handles.pbz);
    %     sdev = std2(ydiff);
    %     maxdiff = max(ydiff);
...
%     sprintf('%s', 'polyplot gca is ',PPgca);
    ...
    ylabel(handles.axes2,'Pb_z','rotation',0);
