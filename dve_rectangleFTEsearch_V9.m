function varargout = dve_rectangleFTEsearch_V9(varargin)
% DVE_RECTANGLEFTESEARCH_V9 MATLAB code for dve_rectangleFTEsearch_V9.fig
%      DVE_RECTANGLEFTESEARCH_V9, by itself, creates a new DVE_RECTANGLEFTESEARCH_V9 or raises the existing
%      singleton*.
%
%      H = DVE_RECTANGLEFTESEARCH_V9 returns the handle to a new DVE_RECTANGLEFTESEARCH_V9 or the handle to
%      the existing singleton*.
%
%      DVE_RECTANGLEFTESEARCH_V9('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in DVE_RECTANGLEFTESEARCH_V9.M with the given input arguments.
%
%      DVE_RECTANGLEFTESEARCH_V9('Property','Value',...) creates a new DVE_RECTANGLEFTESEARCH_V9 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before dve_rectangleFTEsearch_V9_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to dve_rectangleFTEsearch_V9_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help dve_rectangleFTEsearch_V9

% Last Modified by GUIDE v2.5 10-Mar-2012 18:15:15

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @dve_rectangleFTEsearch_V9_OpeningFcn, ...
                   'gui_OutputFcn',  @dve_rectangleFTEsearch_V9_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before dve_rectangleFTEsearch_V9 is made visible.
function dve_rectangleFTEsearch_V9_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to dve_rectangleFTEsearch_V9 (see VARARGIN)

% Choose default command line output for dve_rectangleFTEsearch_V9
...
% UIWAIT makes dve_rectangleFTEsearch_V9 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = dve_rectangleFTEsearch_V9_OutputFcn(hObject, eventdata, handles) 
...
% ===============For some reason, the following code broke in v9.  I
% replaced with a copy/paste from v7.
...
function plotstuff(hObject, eventdata,handles)
    ...
    yminz = min(get(handles.axes1,'Ylim'));
    xminz = min(get(handles.axes1,'Xlim'));
    xmax0r = max(get(handles.axes1,'Xlim'));
    handles.normz=sqrt(handles.bys.^2+handles.bxs.^2);
    maxnormz = max(handles.normz);
    vecx = xminz+(0.1*xmax0r);
    vecy = yminz-0.3*yminz;
    TN=10e9;
    %     maxtexts=strcat(num2str(maxnormz*TN,'%3.1f'),' nT');
    handles.nquiv=quiver(handles.axes1,vecx,vecy,maxnormz,0.0,TN,'w','linewidth',2.0);
