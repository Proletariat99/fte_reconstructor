...
% Last Modified by GUIDE v2.5 24-May-2012 13:37:54
...
%     SET CURRENT AXES HERE (AXES 1)
    ...
    % TextArrow for Vector Here
    lpos = get(gca, 'Position');
    vlx = max(x)*0.3*1e9;   % local calc for x cmpnt of vector
    vly = max(y)*0.3*1e9;   % local calc for y cmpnt of vector
    wlx = lpos(3);
    wly = lpos(4);
    vlm = sqrt(vlx^2 + vly^2);
    yy = [wly*0.1 wly*0.1];
    xx = [wlx*0.2 wlx*0.2+vlm]
    %txtarr = annotation('textarrow',xx,yy,'String','We are here.','FontSize',14);
    disp('')
    axis equal
