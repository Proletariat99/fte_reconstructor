function varargout = dve_rectangleFTEsearch_V9_32(varargin)
% DVE_RECTANGLEFTESEARCH_V9_32 MATLAB code for dve_rectangleFTEsearch_V9_32.fig
%      DVE_RECTANGLEFTESEARCH_V9_32, by itself, creates a new DVE_RECTANGLEFTESEARCH_V9_32 or raises the existing
%      singleton*.
%
%      H = DVE_RECTANGLEFTESEARCH_V9_32 returns the handle to a new DVE_RECTANGLEFTESEARCH_V9_32 or the handle to
%      the existing singleton*.
%
%      DVE_RECTANGLEFTESEARCH_V9_32('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in DVE_RECTANGLEFTESEARCH_V9_32.M with the given input arguments.
%
%      DVE_RECTANGLEFTESEARCH_V9_32('Property','Value',...) creates a new DVE_RECTANGLEFTESEARCH_V9_32 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before dve_rectangleFTEsearch_V9_32_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to dve_rectangleFTEsearch_V9_32_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help dve_rectangleFTEsearch_V9_32

% Last Modified by GUIDE v2.5 24-May-2012 16:29:02

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @dve_rectangleFTEsearch_V9_32_OpeningFcn, ...
                   'gui_OutputFcn',  @dve_rectangleFTEsearch_V9_32_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before dve_rectangleFTEsearch_V9_32 is made visible.
function dve_rectangleFTEsearch_V9_32_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to dve_rectangleFTEsearch_V9_32 (see VARARGIN)

% Choose default command line output for dve_rectangleFTEsearch_V9_32
...
% UIWAIT makes dve_rectangleFTEsearch_V9_32 wait for user response (see UIRESUME)
% uiwait(handles.fig1);


% --- Outputs from this function are returned to the command line.
function varargout = dve_rectangleFTEsearch_V9_32_OutputFcn(hObject, eventdata, handles) 
...
function btn_svpoly_Callback(hObject, eventdata, handles)
    ...
    plot(gca,handles.ax1,handles.yys1,'color',[0.5 0.5 0.5],'linewidth',3);
    set(gca,'ylim',[min(handles.pbz)-0.1 max(handles.pbz)+0.1],'ytick',[0:0.1:1],'fontsize',10);
    ylabel(gca,'p_t','rotation',0);
    xlabel(gca,'A');
...
function btn_save_Callback(hObject, eventdata, handles)
    ...
    set(gca,'NextPlot','replace');
    heada = strcat('TH',handles.sc,datestr(handles.fdna),' to ',datestr(handles.fdnb),' Theta=',num2str(handles.theta),' Phi=',num2str(handles.phi),', w:', num2str(handles.w), ...
    ...
    text(lef+c5,bot-r4,num2str(handles.V0(3),'%3.1f'), 'Fontsize', 10, ...
        'BackgroundColor','w', 'HorizontalAlignment', 'right');
    text(lef+c2,top+up,[heada],'Fontname','arial','Fontsize',...
...
%       Old Save Below -- Reworked into Local above
%     get(get(gcf, 'CurrentAxes'),'Tag')
%     figpos = [1, 1, 800,600];
%     fig2=figure('Name', 'fig2','Position',figpos,...
%         'NumberTitle','on', 'Renderer', 'Zbuffer','RendererMode','manual');
%     set(fig2,'Children',handles.axes1);
%     set(fig2,'CurrentAxes',handles.axes1);
%     sprintf('%s',get(gcbo,'Tag'));
%     dnm = num2str([handles.yr,handles.month,handles.day]);
%     thnm = num2str(round(handles.theta));
%     phnm = num2str(round(handles.phi));
%     tmnm1 = datestr(handles.ta);
