...
function[VHT]=test(doy,ti,tf,sc,source,nev,va_theta,va_phi,theta)
clear all
close all
clc
...
n = 113;			% # of FTEs
nev = 68;			% The event I'm working on (1 is first, not 0) so 1/113, 2/113, etc.
va_theta = 0.0;		% theta for variance analysis
va_phi = 0.0;		% phi for variance analysis
theta = 10.0;	

% USE THIS FORMAT FOR INPUT:
% doy = '2007-06-08';
% ti = '06:30:00';
% tf = '08:30:00';
% sc = 'a';
...
% dhead = 1;          % # of header lines
% n = 113;			% # of FTEs
% nev = 1;            % which FTE we're looking at
...
% TIME STUFF %
% Converting all time to datenumber (mS from 1970)
year = sscanf(doy, '%4c20%*4c',4); 
...
fn = ['th',sc,'_',num2str(strcat(year,month,day)),'_',num2str(strcat(HHi,MMi)),'_',num2str(strcat(HHf,MMf)),'_GSM_v4_',source,'.dat'];
...
ta = f{2}(nev);
tb = f{4}(nev);
sdoy = strcat(num2str(year),'-',num2str(month),'-',num2str(day));
...
ns1 = k(1)-2;                % adds two data points to either side
ns2 = k(end) + 2;            % of FTE
ind = [ns1:ns2];             
ind = ind';


%in = a.data(:,1) >= t1f1 & a.data(:,1) <= t1f2;
%out = a.data(:,1) < t1f1 | a.data(:,1) > t1f2;
%dn = repmat(,n,1)
%ind = datenum(f{2}(nev)) >= datenum(vts1:vts2)
%= f{2}(nev) = a.data(2)
...
%[cx,cy]=size(sec);                         % This is where the problem is.
