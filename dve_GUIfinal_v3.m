...
handles.mvab_theta=acosd(zs0(3));
handles.mvab_phi=atand(abs(zs0(2))/abs(zs0(1)));
...
% need to maybe add GSM zs' components display here. (same as n*orig?)
handles.phi=0.0;
...
% handles.xs = handles.xs/norm(handles.xs);
% handles.ys = handles.ys/norm(handles.ys);
% handles.zs = handles.zs/norm(handles.zs);
guidata(hObject, handles);
...
disp(' ')
if exist('v0gsm')==1
  disp(strcat(' v0gsm        = ',num2str(v0gsm,'%3.1f')))
end
  disp(strcat(' V0           = ',num2str(handles.V0,'%3.1f')))
  disp(strcat('zs0           = ', num2str(handles.zs0, '  %1.3g')))
  disp(strcat(' zs           = ', num2str(handles.zs, '  %1.3g')))
